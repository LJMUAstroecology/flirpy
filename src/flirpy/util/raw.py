...
def raw2temp(raw, meta):
    ...
    tau1 = ATX * np.exp(-np.sqrt(OD / 2) * (ATA1 + ATB1 * np.sqrt(water))) + (
        1 - ATX
    ) * np.exp(-np.sqrt(OD / 2) * (ATA2 + ATB2 * np.sqrt(water)))
    tau2 = tau1
    ...
    raw_refl = (
        PR1 / (PR2 * (np.exp(PB / (RTemp + 273.15)) - PF)) - PO
    ...
    raw_atm1 = (
        PR1 / (PR2 * (np.exp(PB / (ATemp + 273.15)) - PF)) - PO
    )  # radiance from the atmosphere (before the window)
    raw_atm1_attn = (
        (1 - tau1) / E / tau1 * raw_atm1
    )  # attn = the attenuated radiance (in raw units)
